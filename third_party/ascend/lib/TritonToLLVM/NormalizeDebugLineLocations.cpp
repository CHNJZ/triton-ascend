...
bool retargetSyntheticOpToStoreLoc(Operation *op, Location originalLoc) {
  ...
  if (std::optional<Location> storeLoc = getUniqueStoreLoc(candidates)) {
    SourceLine originalLine =
        getSourceLine(canonicalizeSourceLoc(originalLoc));
    SourceLine targetLine =
        getSourceLine(canonicalizeSourceLoc(*storeLoc));
...
void collectSemanticAnchors(Block &block,
  ...
  for (Operation &op : block) {
    ...
    if (!isUserVisibleLoadAnchorOp(&op) &&
        !isUserVisibleStoreAnchorOp(&op) && !isRealMemoryOrCallOp(&op))
      continue;
...
void relocateLoadDebugNops(Operation *root) {
  ...
  root->walk([&](Operation *op) {
    ...
    SourceLine line =
        getSourceLine(canonicalizeSourceLoc(op->getLoc()));
    if (!line)
      return;
  ...
  root->walk([&](Operation *op) {
    ...
    if (op->getBlock() == anchor->getBlock() &&
        op->getNextNode() == anchor) {
